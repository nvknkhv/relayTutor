  ...
  _proto.readFromCacheOrEvaluate = function readFromCacheOrEvaluate(record, field, variables, evaluate, getDataForResolverFragment) {
    ...
    if (linkedRecord == null || this._isInvalid(linkedRecord, getDataForResolverFragment)) {
      ...
      if (field.kind === RELAY_LIVE_RESOLVER) {
        ...
        if (process.env.NODE_ENV !== "production") {
          !!isLiveStateValue(evaluationResult.resolverResult) ? process.env.NODE_ENV !== "production" ? invariant(false, 'Unexpected LiveState value retuned from the non-@live Relay Resolver backing the field "%s". Did you intend to add @live to this resolver?.', field.path) : invariant(false) : void 0;
        }
